...
**Обновление 21 сентября — меню.** В M212 добавлены `Single Player`,
`Multiplayer → Co-op campaign / Versus → Create server / Connect` с вводом
адреса, порта и имени. Локальные прямые URL-проверки подтвердили загрузку
`PrivetDr`, listen-серверы обоих режимов и подключение второго игрока; клики
по меню, изображение и ввод на двух физических ПК ещё не приняты. Сборка
