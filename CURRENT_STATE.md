...
После пользовательского сообщения о зависающих катсценах найдено расхождение:
меню запускало `Game.exe?listen`, а прежние проверенные co-op тесты — выделенный
`UCC.exe` и отдельных клиентов. В логах listen-хоста Ch1/Ch2 команды отправлялись
в незахваченную `BaseCam2`; следующая сцена сталкивалась с оставшимся захватом.
Меню переключено на выделенный сервер и отдельное окно хоста; локальные серверные
прогоны Ch1 и Ch2 завершили вступления без `MP_CUT_ERROR`. Это не доказывает
визуальный результат или прохождение на двух физических ПК. Подробный аудит:
`docs/audits/20260921_MENU_CUTSCENE_AUDIT.md`.
