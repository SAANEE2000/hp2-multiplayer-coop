...
Цель: проверить подключение двух настоящих клиентов, личные камеры/управление, intro Ch1 и четыре заклинания. Это **диагностическая сборка, не завершённый Milestone 1**. Health/death/respawn, ledge/mount, campaign travel/save и disconnect во время сцены пока отмечать **NOT RUN**. Полный протокол: [LOCAL_TEST_PROTOCOL.md](LOCAL_TEST_PROTOCOL.md).
...
```powershell
$artifact = '.local\distribution\hp2-test-build-20260920-163651-880-e49de1c8.zip'
...
ZIP содержит одну сборку HGame.u + M212Share.u; его SHA-256: `8b89cf7ff5263d083183cdc98893bf0d93d2992ea94423b490c4848c1fd956f5`. Runtime JSON SHA-256: `cebbaa5db04fad8b87ae0df4798858df2819044d79f0a9e70c8610db133b4fd0`. После import не пересобирайте пакеты независимо на двух ПК: UCC GUID может различаться. В этом artifact `sourceDirty=true`; source commit/ZIP не являются утверждением, что бинарники собраны из чистого commit. Точные пакеты определяются записанными хешами.
