...
param(
    ...
    [ValidateSet('None','Health','Lumos','Pickup','PickupNet','AIInspect','AICombat')][string]$RuntimeProbe = 'None',
...
if ($CapturedAuthorityDiagnostic -and ($Mode -ne 'Coop' -or $Role -ne 'Host' -or $mapName -ine 'Ch1Rictusempra' -or $TestStage -ne 'RictusempraLessonComplete' -or $RuntimeProbe -notin @('None','AIInspect','AICombat'))) {
